...
def search_ostatok(m):
    ostatki = []
    for i in range(6*m):
        ostatki.append(fib(i) % m)
        if (i > 1 and ostatki[i] == 1 and ostatki[i-1] == 0):
            break
    return ostatki[n % (len(ostatki) - 2)]

print(search_ostatok(m))
# Это немного медленная реализация. Тормозит для очень больших n.